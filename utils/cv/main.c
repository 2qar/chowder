 ...
 * in that region.
 *
 * Ex. "cv r.0.0.mca 0,0" will print every section in the first chunk in
 * r.0.0.mca.
 *
...
int main(int argc, char **argv)
{
	...
	if (world_coords) {
		...
		if (pos->section >= c->sections_len) {
			...
			print_section_func print_section =
			    pretty_print ? pretty_print_section
					 : raw_print_section;
			print_section(c->sections[pos->section], pos);
		} else {
			print_chunk_func print_chunk =
			    pretty_print ? pretty_print_chunk : raw_print_chunk;
			print_chunk(c, pos);
		}
	}
...
void usage()
{
	printf(
	    ...
	    "  cv [options] region_file wx,y,z       Print the block in this "
	    "region at the global x,y,z coords\n"
	    "\noptions:\n"
	    "  -R: instead of pretty-printing sections, print the following "
	    "line-seperated values:\n"
	    "        section Y-index\n"
	    "        palette length\n"
	    "        palette entries, one per line\n"
	    "        bits per block\n"
	    "        blockstates, one per line, compacted into unsigned 64-bit "
...
void print_sections(const struct chunk *c, struct pos *p,
		    print_section_func print_section)
{

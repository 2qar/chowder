...
void read_cv_section(FILE *out_file, struct section *section)
{
	...
	if (section->bits_per_block != -1) {
		...
		section->blockstates =
		    calloc(blockstates_len, sizeof(uint64_t));
	}
...
{
	return s1->y == s2->y && s1->palette_len == s2->palette_len
	       && (s1->palette_len == -1
		   || array_equal(s1->palette, s2->palette,
				  sizeof(int) * s1->palette_len))
	       && s1->bits_per_block == s2->bits_per_block
	       && (s1->bits_per_block == -1
		   || array_equal(s1->blockstates, s2->blockstates,
				  sizeof(uint64_t)
				      * BLOCKSTATES_LEN(s1->bits_per_block)));
}

bool sections_equal(size_t sections_len, struct section *s1[16],
		    struct section *s2[16])
{
	size_t i = 0;
...
int main()
{
	...
	enum anvil_err err =
	    anvil_get_chunk(region_file, block_table, 0, 0, &anvil_chunk);
	assert(err == ANVIL_OK);
